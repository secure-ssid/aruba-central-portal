...
def _get_utilization_trend(device_type, serial, metric, endpoint_path, extra_fields=None, unit=None):
    ...
    try:
        ...
        logger.info(f"Fetching {device_type} {metric}: {endpoint_path} for serial: {serial} with params: {params}")
        ...
        if "404" in error_str or "Not Found" in error_str:
            error_response["suggestion"] = f"Verify the serial number and that the {device_type} exists in your Central account"
...
@monitoring_bp.route('/api/monitoring/gateways/<serial>/uplinks/<link_tag>/throughput', methods=['GET'])
@require_session
...
@monitoring_bp.route('/api/monitoring/gateways/<serial>/uplinks/<link_tag>/wan-availability', methods=['GET'])
@require_session
...
@monitoring_bp.route('/api/monitoring/gateways/<serial>/ports/<port_number>/throughput', methods=['GET'])
@require_session
...
@monitoring_bp.route('/api/monitoring/gateways/<serial>/tunnel/<tunnel_name>/throughput', methods=['GET'])
@require_session
...
def get_gw_wan_interfaces(serial):
    ...
    try:
        ...
        response = aruba_client.get(f'/network-monitoring/v1alpha1/gateways/{serial}/wan-interfaces')
