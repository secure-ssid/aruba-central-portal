...
def main():
    ...
    # Only pass parameters that ArubaClient expects
    client_params = {
    ...
    if aruba_config.get("username"):
        client_params["username"] = aruba_config["username"]
    if aruba_config.get("password"):
        client_params["password"] = aruba_config["password"]

    client = ArubaClient(**client_params)
